...
notManyGasStation :- findall(install(C), city(C), Lits), maxStations(K), atMost(K, Lits), fail.
notManyGasStation.

cityPairs :- city(C1), city(C2), (road(C1,C2) ; road(C2,C1)), writeOneClause([install(C1), install(C2)]), fail.
cityPairs.
...
displaySol(M) :- write('Ciudades: '),
        nl,
        city(C), member(install(C), M), write(C), nl, fail.
