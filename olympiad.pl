...
excludeNotFriends :-  student(A), student(B), A =\= B,  \+mutualFriends(A, B), atMost(1, [chosen(A), chosen(B)]), fail.
excludeNotFriends.

mutualFriends(A, B) :-  student(A), student(B), friends(A, B), friends(B,A).

differentTopics :- student(A), student(B), A=\=B, expertise(A, E), expertise(B, E), atMost(1,[chosen(A), chosen(B)]), fail.
differentTopics.
