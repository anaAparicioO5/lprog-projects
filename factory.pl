...
satVariable( active(T,H) ) :- task(T), hour(H). % "task T esta activa a l'hora h"
...
writeClauses(MaxHours) :-
    ...
    creatActive(MaxHour),
    availableResource(MaxHours), 
    
    true, !.
writeClauses(_) :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachTaskStartsOnce(MaxHour) :- 
        task(T), 
        duration(T,D), 
        HD is MaxHour - D+1, 
...
createActive(MaxHour) :-
        task(T),
        between(1, MaxHour, H),
        duration(T, D),
        between(0, D, A),
        Ax is H + A,
        writeClauses([-start(T,H)], [active(T,Ax)]),
        fail.
creatActive(_).


availableResource(MaxHour) :-
        hour(H),
        between(1, MaxHour, H),
        resourceUnits(R, X),
