...
%misma paridad coste 0, diferente paridad coste 1
cost(C1, C2, 0) :- C1 =\= C2, C1 mod 2 =:= C2 mod 2, !.
cost(_,_, 1).
...
%comprova que el cost no superi el cost maxim
%exceededCost :- 
        %maxCost(Max), 
        %findall(Tupla(C1, C2, Coste), (city(C1), city(C2),cost(C1, C2, Coste)), ListCoste),
        %ListCoste =< Max,
        %fail.
%exceededCost.

setMaxCost :-
    maxCost(Max),                    % Obtenemos el costo máximo permitido
    findall(CostLiteral, 
            (city(I), city(J),       % Para cada par de ciudades I y J...
             cost(I, J, 1),          % Verificamos si el costo entre I y J es 1
             position(P), 
             position(P1), 
             P1 is P + 1,            % Consideramos posiciones consecutivas en la ruta
             adjacency(I, Adj), 
             member(J, Adj),         % Si J es adyacente a I
             expressAnd(CostLiteral, [visited(I, P), visited(J, P1)])
            ), 
            CostLiterals),           % Recopilamos los literales de costos 1
    atMost(Max, CostLiterals).       % Limitamos el número de trayectos con costo 1



%cada ciutat nomes visitada un cop
alreadyVisited :-
        city(C),
...
ensureAdjacency :-
        numCities(N),
        forall(between(0, N-1), Pos), allowedNextMov(Pos),
        visited(city, Pos),
        writeOneClause([visited(city, Pos)]).

%comprovar el seguent moviment
allowedNextMov(Pos) :-
        visited(C1, Pos),
        position(NextP), NextP is Pos + 1,
        visited(C2, NextP),
        adjacency(C1, adjacents),
        members(C2, adjacents),
        fail.
allowedNextMov.
